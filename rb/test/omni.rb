...
module PublicApisDatabaseOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if PublicApisDatabaseOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = PublicApisDatabaseOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| PublicApisDatabaseOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || PublicApisDatabaseOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = PublicApisDatabaseSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = PublicApisDatabaseError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { PublicApisDatabaseOmni.errify(err) }

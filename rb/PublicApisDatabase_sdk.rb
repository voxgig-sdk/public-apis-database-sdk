...
require_relative 'PublicApisDatabase_types'
...
class PublicApisDatabaseSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PublicApisDatabaseHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PublicApisDatabaseHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PublicApisDatabaseFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PublicApisDatabaseUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PublicApisDatabaseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PublicApisDatabaseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PublicApisDatabaseHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PublicApisDatabaseError.new(
        "#{op}_allow",
        "PublicApisDatabaseSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PublicApisDatabaseError => err
    ...
    if fetched.is_a?(Hash)
      status = PublicApisDatabaseHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PublicApisDatabaseError.new(
        "graphql_error", "PublicApisDatabaseSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PublicApisDatabaseSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

...
module PublicApisDatabaseUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PublicApisDatabaseResponse.new({})
    ...
    if fetch_err
      ...
      response = PublicApisDatabaseResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PublicApisDatabaseResponse.new(fetched)
    else

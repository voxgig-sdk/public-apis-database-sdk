# PublicApisDatabase SDK ApI entity
...
class ApIEntity
  ...
  def data_set(args)
    if args
      @_data = PublicApisDatabaseHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PublicApisDatabaseHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [ApI, Hash] the loaded ApI; raises PublicApisDatabaseError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PublicApisDatabaseHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ApI>, Array] the matching ApI items; raises PublicApisDatabaseError on failure

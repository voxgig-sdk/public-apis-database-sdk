...
// PublicApisDatabase SDK pipeline test
//
...
{
    private static function utility(): PublicApisDatabaseUtility
    {
        return new PublicApisDatabaseUtility();
    }
    ...
    private static function ctx(array $over = []): PublicApisDatabaseContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PublicApisDatabaseContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PublicApisDatabaseError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PublicApisDatabaseMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PublicApisDatabaseMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PublicApisDatabaseMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicApisDatabaseSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PublicApisDatabaseMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PublicApisDatabaseResponse([]),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        [, $err] = PublicApisDatabaseMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec([]),
            'response' => null,
            'result' => new PublicApisDatabaseResult([]),
        ]);
        [, $err] = PublicApisDatabaseMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec(['step' => 's']),
            'response' => new PublicApisDatabaseResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        [, $err] = PublicApisDatabaseMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec(['step' => 's']),
            'response' => new PublicApisDatabaseResponse(self::resp(200, ['v' => 1])),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        [, $err] = PublicApisDatabaseMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec(['step' => 's']),
            'response' => new PublicApisDatabaseResponse(self::resp(200, ['v' => 2])),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PublicApisDatabaseMakeResponse::call($ctx);
    ...
    {
        $preset = new PublicApisDatabaseResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec([]),
            'response' => new PublicApisDatabaseResponse([]),
            'result' => new PublicApisDatabaseResult([]),
        ...
        [$got, $err] = PublicApisDatabaseMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PublicApisDatabaseResult([])]);
        [, $err] = PublicApisDatabaseMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PublicApisDatabaseSpec([]), 'result' => null]);
        [, $err] = PublicApisDatabaseMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicApisDatabaseSpec(['step' => 's']),
            'result' => new PublicApisDatabaseResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicApisDatabaseMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicApisDatabaseSpec(['step' => 's']),
            'result' => new PublicApisDatabaseResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PublicApisDatabaseOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicApisDatabaseMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicApisDatabaseResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PublicApisDatabaseSpec([]),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PublicApisDatabaseMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicApisDatabaseMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PublicApisDatabaseError('boom', 'boom');
        $utility->fetcher = function (PublicApisDatabaseContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicApisDatabaseSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PublicApisDatabaseMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PublicApisDatabaseContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PublicApisDatabaseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PublicApisDatabaseResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PublicApisDatabaseMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PublicApisDatabaseContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicApisDatabaseSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PublicApisDatabaseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PublicApisDatabaseResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PublicApisDatabaseSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PublicApisDatabaseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PublicApisDatabaseMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PublicApisDatabaseMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PublicApisDatabaseResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PublicApisDatabaseDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PublicApisDatabaseResult(['ok' => false])]);
        $this->expectException(PublicApisDatabaseError::class);
        PublicApisDatabaseDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PublicApisDatabaseResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PublicApisDatabaseMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PublicApisDatabaseMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PublicApisDatabaseBaseFeature();
        $b = new PublicApisDatabaseBaseFeature();
        PublicApisDatabaseFeatureAdd::call($ctx, $a);
        PublicApisDatabaseFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PublicApisDatabaseBaseFeature
    {
        $f = new PublicApisDatabaseBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PublicApisDatabaseFeatureAdd::call($ctx, self::named_feature('a'));
        PublicApisDatabaseFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        PublicApisDatabaseFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        PublicApisDatabaseFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        PublicApisDatabaseFeatureAdd::call($ctx, $replace);
        ...
        PublicApisDatabaseFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PublicApisDatabaseContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PublicApisDatabaseSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicApisDatabasePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PublicApisDatabasePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PublicApisDatabaseResponse(['headers' => null]),
            'result' => new PublicApisDatabaseResult([]),
        ]);
        PublicApisDatabaseResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PublicApisDatabaseResultBody::call($ctx);

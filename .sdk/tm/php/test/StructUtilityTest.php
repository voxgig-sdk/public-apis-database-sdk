...
// PublicApisDatabase SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PublicApisDatabaseOmni::makeStructRunner(
    ...
    {
        return PublicApisDatabaseOmni::objmodel(PublicApisDatabaseOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return PublicApisDatabaseOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return PublicApisDatabaseOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PublicApisDatabaseOmni::structfix($now, Struct::undef()))
                        !== json_encode(PublicApisDatabaseOmni::structfix($val, Struct::undef()))) {

...
//    only - a typed PublicApisDatabaseContext there would make every ctx
...
final class PublicApisDatabaseOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PublicApisDatabaseContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new PublicApisDatabaseSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new PublicApisDatabaseResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new PublicApisDatabaseError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new PublicApisDatabaseResponse($clean['response']);
    ...
    public static function err_from_map($m): ?PublicApisDatabaseError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PublicApisDatabaseError(is_string($code) ? $code : '', $msg);
    }

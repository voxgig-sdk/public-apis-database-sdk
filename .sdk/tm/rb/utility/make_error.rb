...
module PublicApisDatabaseUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PublicApisDatabaseContext.new({}, nil)
    end
    op = ctx.op || PublicApisDatabaseOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PublicApisDatabaseResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PublicApisDatabaseError) ? err.msg : err.to_s
    ...
    sdk_err = PublicApisDatabaseError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PublicApisDatabaseError)

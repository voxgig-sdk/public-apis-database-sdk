...
require("public-apis-database_types")
...
local PublicApisDatabaseSDK = {}
PublicApisDatabaseSDK.__index = PublicApisDatabaseSDK
...
PublicApisDatabaseSDK._make_feature = _make_feature


function PublicApisDatabaseSDK.new(options)
  local self = setmetatable({}, PublicApisDatabaseSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PublicApisDatabaseSDK:options_map()
...
function PublicApisDatabaseSDK:get_utility()
  return Utility.copy(self._utility)
end


function PublicApisDatabaseSDK:get_root_ctx()
  return self._rootctx
end


function PublicApisDatabaseSDK:prepare(fetchargs)
...
function PublicApisDatabaseSDK:direct(fetchargs)
...
function PublicApisDatabaseSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function PublicApisDatabaseSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "PublicApisDatabaseSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function PublicApisDatabaseSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PublicApisDatabaseSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PublicApisDatabaseSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PublicApisDatabaseSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PublicApisDatabaseSDK.new(sdkopts)
  sdk.mode = "test"

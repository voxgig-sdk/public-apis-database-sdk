...
describe("ApIDirect", function()
  it("should direct-list-ap_i", function()
    local setup = ap_i_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ap_i", setup.live and "live" or "unit")
  ...
  it("should direct-load-ap_i", function()
    local setup = ap_i_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-ap_i", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function ap_i_direct_setup(mockres)

...
func TestApIEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PUBLICAPISDATABASE_TEST_AP_I_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ap_iBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PUBLICAPISDATABASE_TEST_AP_I_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PUBLICAPISDATABASE_TEST_AP_I_ENTID"])
	...
	if env["PUBLICAPISDATABASE_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewPublicApisDatabaseSDK(core.ToMapAny(mergedOpts))
	}

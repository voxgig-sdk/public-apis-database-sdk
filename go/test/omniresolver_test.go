...
import (
	...
	omni "github.com/voxgig-sdk/public-apis-database-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PublicApisDatabaseError); is {
				out := map[string]any{
					"name":    "PublicApisDatabaseError",
...
func omniCtx(arg any, client *sdk.PublicApisDatabaseSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PublicApisDatabaseSDK); is {
			client = live

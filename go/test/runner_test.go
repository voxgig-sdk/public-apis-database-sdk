...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PUBLICAPISDATABASE_TEST_LIVE") == "TRUE" ||
		os.Getenv("PUBLICAPISDATABASE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PUBLICAPISDATABASE_TEST_EXPLAIN"); explain != "" {

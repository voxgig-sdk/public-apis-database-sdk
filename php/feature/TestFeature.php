...
// PublicApisDatabase SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PublicApisDatabaseTestFeature extends PublicApisDatabaseBaseFeature
{
    ...
    public function init(PublicApisDatabaseContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PublicApisDatabaseContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PublicApisDatabaseParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PublicApisDatabaseContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PublicApisDatabaseContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PublicApisDatabaseParam::call($ctx, $k);

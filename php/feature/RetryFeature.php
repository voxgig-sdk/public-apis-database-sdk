...
// PublicApisDatabase SDK retry feature
...
class PublicApisDatabaseRetryFeature extends PublicApisDatabaseBaseFeature
{
    ...
    public function init(PublicApisDatabaseContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PublicApisDatabaseContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PublicApisDatabaseContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PublicApisDatabaseContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

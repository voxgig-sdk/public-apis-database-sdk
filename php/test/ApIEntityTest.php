...
class ApIEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = PublicApisDatabaseSDK::test($seed, null);
        $seen = iterator_to_array($base->ApI(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = PublicApisDatabaseConfig::make_config();
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PUBLIC_APIS_DATABASE_TEST_AP_I_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ap_i_ref01_data_dt0_loaded = $ap_i_ref01_ent->load($ap_i_ref01_match_dt0, null);
        $ap_i_ref01_data_dt0_load_result = Helpers::to_map(is_object($ap_i_ref01_data_dt0_loaded) && method_exists($ap_i_ref01_data_dt0_loaded, 'data_get') ? $ap_i_ref01_data_dt0_loaded->data_get() : $ap_i_ref01_data_dt0_loaded);
        $this->assertNotNull($ap_i_ref01_data_dt0_load_result);
        $this->assertEquals($ap_i_ref01_data_dt0_load_result["id"], $ap_i_ref01_data["id"]);
...
function ap_i_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("PUBLIC_APIS_DATABASE_TEST_AP_I_ENTID");
    ...
    if ($env["PUBLIC_APIS_DATABASE_TEST_LIVE"] === "TRUE") {
        ...
        $client = new PublicApisDatabaseSDK(Helpers::to_map($merged_opts));
    }

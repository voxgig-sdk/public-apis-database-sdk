...
// PublicApisDatabase SDK feature test
//
...
class FtCtrl extends PublicApisDatabaseControl
{
...
class FtClient
{
    ...
    private PublicApisDatabaseUtility $utility;

    public function __construct(PublicApisDatabaseUtility $utility, array $options)
    {
    ...
    public function get_utility(): PublicApisDatabaseUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PublicApisDatabaseContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PublicApisDatabaseContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PublicApisDatabaseContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PublicApisDatabaseConfig::make_feature($name);
    ...
    private static function buildUrl(PublicApisDatabaseSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PublicApisDatabaseError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PublicApisDatabaseResponse($fetched);
        }
    ...
    private function fail(PublicApisDatabaseContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PublicApisDatabaseContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

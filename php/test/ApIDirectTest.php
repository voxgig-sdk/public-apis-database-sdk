...
{
    public function test_direct_list_ap_i(): void
    {
        $setup = ap_i_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-ap_i", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_ap_i(): void
    {
        $setup = ap_i_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-ap_i", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function ap_i_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new PublicApisDatabaseSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,

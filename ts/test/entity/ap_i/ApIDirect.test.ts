...
describe('ApIDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PublicApisDatabaseSDK({
  ...
  test('direct-load-ap_i', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ap_i', setup.live)) return
  ...
  test('direct-list-ap_i', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ap_i', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PUBLIC_APIS_DATABASE_TEST_AP_I_ENTID']

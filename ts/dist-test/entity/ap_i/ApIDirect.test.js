...
(0, node_test_1.describe)('ApIDirect', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('PUBLIC_APIS_DATABASE_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.PublicApisDatabaseSDK({
    ...
    (0, node_test_1.test)('direct-load-ap_i', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-ap_i', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-ap_i', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-ap_i', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.PUBLIC_APIS_DATABASE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.PublicApisDatabaseSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['PUBLIC_APIS_DATABASE_TEST_AP_I_ENTID'];
    ...
    const client = new __1.PublicApisDatabaseSDK({
        base: 'http://localhost:8080',

...
const __1 = require("..");
const SDK_NAME = 'PublicApisDatabaseSDK';
...
const TEST_SEED = { "entity": { "ap_i": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PublicApisDatabaseSDK\b/.test(code) ||
        /\bPublicApisDatabaseSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PublicApisDatabaseSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PublicApisDatabaseSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

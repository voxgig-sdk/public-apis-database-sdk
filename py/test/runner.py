# PublicApisDatabase SDK test runner
...
class PublicApisDatabaseTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PublicApisDatabaseTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PublicApisDatabaseTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PublicApisDatabaseTestRunner.getenv("PUBLIC_APIS_DATABASE_TEST_LIVE")
        override = PublicApisDatabaseTestRunner.getenv("PUBLIC_APIS_DATABASE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PublicApisDatabaseTestRunner.getenv(key)
        ...
        explain = PublicApisDatabaseTestRunner.getenv("PUBLIC_APIS_DATABASE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PublicApisDatabaseTestRunner._test_control is not None:
            return PublicApisDatabaseTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PublicApisDatabaseTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PublicApisDatabaseTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PublicApisDatabaseTestRunner.load_test_control()
...
def load_env_local():
    PublicApisDatabaseTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return PublicApisDatabaseTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PublicApisDatabaseTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PublicApisDatabaseTestRunner.load_test_control()


def live_client_options():
    return PublicApisDatabaseTestRunner.live_client_options()


def live_delay_ms():
    return PublicApisDatabaseTestRunner.live_delay_ms()

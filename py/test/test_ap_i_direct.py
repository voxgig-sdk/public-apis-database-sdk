...
class TestApIDirect:
    ...
    def test_should_direct_list_ap_i(self):
        setup = _ap_i_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ap_i", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_ap_i(self):
        setup = _ap_i_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ap_i", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _ap_i_direct_setup(mockres):
    ...
    if live:
        ...
        client = PublicApisDatabaseSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,

...
AQL-driven command-line client **and** interactive REPL for the PublicApisDatabase
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/public-apis-database-cli)
make build

# 2. See usage (words, entities, env vars)
./public-apis-database-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/public-apis-database-cli
   ```
...
Run with no arguments to open a REPL (prompt `public-apis-database>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `ap_i`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/public-apis-database-cli`. |
...
(`./public-apis-database-cli <expr>`) and interactively (the REPL), and expressions compose the

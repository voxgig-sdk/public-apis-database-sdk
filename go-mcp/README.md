# public-apis-database-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PublicApisDatabase SDK as
two agent tools — `public-apis-database_list` and `public-apis-database_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/public-apis-database-mcp)
make build
...
claude mcp add --scope user public-apis-database \
  -- /absolute/path/to/public-apis-database-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./public-apis-database-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/public-apis-database-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user public-apis-database \
     -- "$PWD"/dist/*/public-apis-database-mcp -transport stdio
   ```
...
### Call the `public-apis-database_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `public-apis-database_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `public-apis-database_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PUBLIC_APIS_DATABASE_APIKEY` | API key sent with every request. |
| `PUBLIC_APIS_DATABASE_BASE` | Optional override of the API base URL. |
...
```sh
./public-apis-database-mcp -transport http -addr :18080 &

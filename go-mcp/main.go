// Command PublicApisDatabase-mcp is an MCP server exposing the PublicApisDatabase
// SDK as MCP tools.
//
// Two tools are registered:
//
//	PublicApisDatabase_list  — list records of an entity
//	PublicApisDatabase_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: PUBLIC_APIS_DATABASE_APIKEY carries the API key and
	// PUBLIC_APIS_DATABASE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PUBLIC_APIS_DATABASE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PUBLIC_APIS_DATABASE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPublicApisDatabaseSDK(opts)
